...
export function groupByDayWithId(unsortedAppointments: IAppointment[]) {
  const appointments: ISortedAppointmentsWithID[] = []
  unsortedAppointments.forEach((appointment) => {
    const formatedDay = `${week_days[getDay(new Date(appointment.date))]}, ${getDate(new Date(appointment.date))}`
    ...
    if (dayFound < 0) {
      appointments.push({ day: new Date(appointment.date), formatedDay, times: [{ time: new Date(appointment.date), formattedTime, id: appointment.id }] })
    } else {
      appointments[dayFound].times.push({ time: new Date(appointment.date), formattedTime, id: appointment.id })
