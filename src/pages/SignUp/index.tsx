...
const SignUp: React.FC = () => {
  ...
  const handleSelectCountry = useCallback((e: ChangeEvent<HTMLSelectElement>) => {
  ...
  const handleSubmitForm = useCallback(({ name, email, password, phone }) => {
    const user = {
      name, email, password, phone, country: countries.find(country => country.value === selectedCountry)?.label, state: states.find(state => {
      ...
      <Content>
        <div>
          ...
          <Form onSubmit={handleSubmitForm}>
            ...
            <Input name='email' placeholder="E-mail" required type='email' autoComplete='on' />
            <Input name='phone' placeholder="Telefone: (00) 98765 4321" required type="tel" />
            <Input name='password' placeholder="Senha" required type={passwordVisible ? "text" : "password"} icon={passwordVisible ? (
              <FiEyeOff
            ...
            <Select value={selectedCountry} label='País' options={countries} name='country' onChange={handleSelectCountry} required />

            <Select value={selectedState ? selectedState : ''} disabled={selectedCountry !== 'BRA'} label='Estado' options={states} name='state' onChange={handleSelectState} />

            <Select value={selectedCity} disabled={!selectedState} label='Cidade' options={cities} name='city' onChange={handleSelectCity} />
